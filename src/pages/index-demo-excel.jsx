...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    const loadSheetData = async () => {
      try {
        ...
        setPlaceNames(sheetData.map((row) => row[0])); // Assuming the place names are in the first column
      } catch (error) {
  ...
  }, [loading, fetchPlaceDetails, query]);
  ...
  const fetchRandomMeal = () => {
    // Randomly select a new query from the data loaded from Google Sheets
